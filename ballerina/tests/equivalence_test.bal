...
function testJsonRpcAndGrpcReturnIdenticalSendMessageResult() returns error? {
    ...
    Task|Message jsonRpcResult = check jsonRpcClient->sendMessage({message: {messageId: "m1", role: ROLE_USER, parts: [{text: "hi"}]}});
    ...
    Task|Message grpcResult = check grpcClient->sendMessage({message: {messageId: "m1", role: ROLE_USER, parts: [{text: "hi"}]}});
...
function testJsonRpcAndGrpcReturnIdenticalNotFoundErrorType() returns error? {
    ...
    Client jsonRpcClient = check new (getServerBaseUrl());
    Task|error jsonRpcResult = jsonRpcClient->getTask({id: "missing"});

    setNextGrpcError(error grpc:NotFoundError("not found"));
    GrpcClient grpcClient = check new (getServerBaseUrl());
    Task|error grpcResult = grpcClient->getTask({id: "missing"});
